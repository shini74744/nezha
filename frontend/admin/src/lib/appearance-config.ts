...
export type FeatureDefinition={key:string;title:string;description:string;defaults:Feature;labels:Record<string,string>;constraints:Record<string,number[]>};
export const definitions=manifestData as unknown as FeatureDefinition[];
export const defaults=():AppearanceConfig=>({version:1,enabled:false,features:Object.fromEntries(definitions.map(d=>[d.key,JSON.parse(JSON.stringify(d.defaults))]))});
export function normalize(raw?:string|AppearanceConfig):AppearanceConfig{
 const base=defaults();if(!raw)return base;
...
function check(key:string,value:any,sample:any):void{
 ...
 if(typeof value==="string"){if(value.length>8192||value.includes("\0"))throw Error(key+" 文本无效");if(value&&urlKey.test(key)){const u=new URL(value);if(!["http:","https:"].includes(u.protocol)||!u.host||u.username||u.password)throw Error(key+" 只允许不含账号密码的 HTTP/HTTPS 地址")}}
}
export function validate(config:AppearanceConfig):string{
 try{
  for(const d of definitions){const f=config.features[d.key];check(d.key,f,d.defaults);for(const [key,range]of Object.entries(d.constraints))if(f[key]<range[0]||f[key]>range[1])throw Error(d.title+"："+(d.labels[key]||key)+" 超出范围 "+range.join("–"));for(const key of ["count","mobileCount","selection","nightStart","nightEnd"])if(typeof f[key]==="number"&&!Number.isInteger(f[key]))throw Error(key+" 必须是整数")}
  if(!/T.*(?:Z|[+-]\d\d:\d\d)$/.test(config.features.runtime.startDate)||Number.isNaN(Date.parse(config.features.runtime.startDate)))throw Error("网站运行起始时间须包含有效时区");
