...
export const validClockColor = (value: unknown): value is string =>
...
export function validateGreetingClock(features: Record<string, any>) {
    ...
    for (const rule of rules) {
        if (!validTime(rule.start) || !validTime(rule.end))
            throw Error("问候时段须使用有效的 HH:mm 时间")
        if (rule.messages.length > 256) throw Error("每个时段最多 256 条问候语")
    }
    for (const key of clockColorKeys)
        if (!validClockColor(features.clock[key])) throw Error("时钟颜色须为 #RRGGBB，例如 #ff2828")
}
...
export function mixClockColor(start: string, end: string, ratio: number): string {
    ...
    const from = rgb(start),
        to = rgb(end),
        t = Math.max(0, Math.min(1, ratio))
    return (
        "rgb(" +
        from.map((value, index) => Math.round(value + (to[index] - value) * t)).join(", ") +
        ")"
    )
}
