// @ts-nocheck
// Migrated source for the built-in asteroids feature; resources are owned by FeatureScope.
export function asteroids(scope, config) {
const window = scope.window; const document = scope.document;
scope.listen(document, "keydown", event => {
  if (event.key === "Escape") scope.dispose();
});
const hadBlinkClass = document.body.classList.contains("ASTEROIDSBLINK");
scope.own(() => {
  if (!hadBlinkClass) document.body.classList.remove("ASTEROIDSBLINK");
});
...
function Asteroids() {
  ...
  const playerWidth = 20,
    playerHeight = 30;
  const playerVerts = [[-1 * playerHeight / 2, -1 * playerWidth / 2], [-1 * playerHeight / 2, playerWidth / 2], [playerHeight / 2, 0]];
  ...
  function addStylesheet(selector, rules) {
    const stylesheet = scope.createElement("style");
    ...
    scope.append(document.getElementsByTagName("head")[0], stylesheet);
  }
  ;
  function removeStylesheet(name) {
    const stylesheet = document.getElementById(name);
    if (stylesheet) {
      scope.detach(stylesheet.parentNode, stylesheet);
    }
  }
  ;
  this.gameContainer = scope.createElement("div");
  this.gameContainer.className = "ASTEROIDSYEAH";
  scope.append(document.body, this.gameContainer);
  this.canvas = scope.createElement("canvas");
  this.canvas.setAttribute("width", w);
  this.canvas.setAttribute("height", h);
  this.canvas.className = "ASTEROIDSYEAH";
  Object.assign(scope.styleOf(this.canvas), {
    width: w + "px",
    height: h + "px",
    position: "fixed",
    top: "0px",
  ...
  scope.listen(this.canvas, "mousedown", function (e) {
    ...
    scope.append(document.body, message);
  ...
  const eventResize = function () {
    ...
    Object.assign(scope.styleOf(that.canvas), {
      display: "block",
      width: w + "px",
      height: h + "px"
    });
  };
  scope.listen(window, "resize", eventResize, false);
  scope.append(this.gameContainer, this.canvas);
  this.ctx = this.canvas.getContext("2d");
  this.ctx.fillStyle = "black";
  this.ctx.strokeStyle = "black";
  if (!document.getElementById("ASTEROIDS-NAVIGATION")) {
    this.navigation = scope.createElement("div");
    this.navigation.id = "ASTEROIDS-NAVIGATION";
    this.navigation.className = "ASTEROIDSYEAH";
    Object.assign(scope.styleOf(this.navigation), {
    ...
    scope.append(this.gameContainer, this.navigation);
    this.points = scope.createElement("span");
    this.points.id = "ASTEROIDS-POINTS";
    scope.styleOf(this.points).font = "28pt Arial, sans-serif";
    scope.styleOf(this.points).fontWeight = "bold";
    this.points.className = "ASTEROIDSYEAH";
    scope.append(this.navigation, this.points);
  } else {
  ...
  scope.listen(document, "keyup", eventKeyup, false);
  ...
  ;
  scope.setTimeout(updateFunc, 1000 / FPS);
    ...
    if (this.dying.length) {
      for (let i = this.dying.length - 1; i >= 0; i--) {
        try {
          ...
          scope.styleOf(this.dying[i]).display = "none";
        } catch (e) {}
      }
      setScore();
      this.dying = [];
